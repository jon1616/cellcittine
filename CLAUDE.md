...
Messaggi "leggeri" (non di fase, non rimandati a chi rientra): `ready` (ospite → host → tutti, chi ha il conto alla rovescia a schermo), `presence` (ospite → host, app in secondo piano → campo `away` nei partecipanti), `progress` (host → tutti a ogni punteggio ricevuto: chi ha finito, mostrato nella schermata "In attesa degli altri…" dei minigiochi con createShell), `react` (ospite → host → tutti: faccina che vola sulla riga con `data-id`), `link` (ospite → host ogni 5 s se cambia: qualità della linea good/slow/bad → campo `link` nei partecipanti). Chi entra a manche iniziata riceve lo `start` con `spectate: true` → schermata "spectate" (guarda, con i risultati in diretta) e gioca dalla manche dopo. `?cid=…` nell'indirizzo impone l'id del telefono (solo per il tester, che apre host e ospite nello stesso browser).
Prima volta di un minigioco per qualcuno in stanza: lo "start" ha `startAt: null`, il conto parte col messaggio `go` (host → tutti) quando tutti i partecipanti presenti hanno premuto "Pronto" (`ready`) o dopo 90 s; l'host aggiorna anche `lastBroadcast.startAt` per chi rientra.
Da soli: `ch.adaptive` (Giro veloce) sceglie la difficoltà per manche da `stats.adaptiveDifficulty` (stelle già prese); le stelle di ogni minigioco stanno in stats (`starsFor`, `totalStars`).
Prova subito a serie (`ch.series` = "lunga" | "infinita", scelta salvata in `storage.getQuickSeries`): partite dello stesso minigioco una dopo l'altra; `publishResults` salta la schermata dei risultati e passa da `seriesStep` (statistiche con `recordMyRound` di results.js, avviso, `nextRound`); con "infinita" `ch.total` è Infinity e `nextRound` aggiunge una partita alla volta; `stopSeries` (pulsante `.series-end`) chiude: tra una partita e l'altra subito, durante la partita fa scadere il conto alla rovescia con `shell.endTimerNow()` (il minigioco chiude col punteggio di ora; se non manda il punteggio entro 2,5 s si chiude comunque senza contarla).
